# Wave 2 — Payment Page Agent Brief

**Your file:** `src/app/payment/page.tsx`
**Route:** `/payment`
**Constraint:** Do not modify any other file. Only write `src/app/payment/page.tsx`.
...
2. `src/components/brief/ModalsAndOverlays.tsx` — `ConfirmJoinSheet` (the row layout inside is your fee breakdown reference)
3. `src/components/brief/FormElements.tsx` — input field styles
4. `src/components/ui/input.tsx`, `src/components/ui/label.tsx`, `src/components/ui/button.tsx`
5. `src/lib/mock-data.ts` — mockLobbies[0] (Alex T., $12 fee), mockPlayer
...
Show a mock Stripe-style checkout for joining a lobby. No real Stripe SDK —
just static form fields and a simulated pay button.

The lobby being paid for: use `mockLobbies[0]` (Alex T., Saturday, Doncaster, $12 fee).
The split fee: court fee is AUD $24 split between 2 players = **AUD $12.00 per player**.
Service fee: AUD $1.20 (10%).
**Total: AUD $13.20.**
...
[Payment form card]
[Pay button]
[Success state — replaces pay button when paid]
```
...
### Section 2: Lobby summary card

Show the lobby details the user is about to pay for.
...
### Section 4: Payment form card

No real validation needed — just controlled inputs.
...
- Do not modify any shared file — only `src/app/payment/page.tsx`
